...
CJS_Value::operator CJS_Object *() const
{
	...
	v8::Local<v8::Object>	pObj = JS_ToObject(m_pValue);
	return (CJS_Object*)JS_GetPrivate(m_isolate, pObj);
}

CJS_Value::operator v8::Local<v8::Object>() const
{
...
v8::Local<v8::Value> CJS_Value::ToJSValue()
{
	return m_pValue;
}


CJS_Value::operator v8::Local<v8::Array>() const
{
	if (IsArrayObject())
		return v8::Local<v8::Array>::Cast(JS_ToObject(m_pValue));
...
void CJS_Array::GetElement(unsigned index,CJS_Value &value)
{
	if (m_pArray.IsEmpty())
		return;
	v8::Local<v8::Value>  p = JS_GetArrayElemnet(m_pArray,index);
